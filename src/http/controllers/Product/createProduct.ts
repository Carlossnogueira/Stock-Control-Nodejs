...
export async function createProduct(request: FastifyRequest, reply: FastifyReply) {
    ...
    const productSchema = z.object({
        ...
        min_quantity: z.coerce.number()
            .min(1, 'Min one of this product')
            .max(999, 'Max 999 of this product'),
        max_quantity: z.coerce.number()
            .min(1, 'Min one of this product')
            .max(999, 'Max 999 of this product'),
        price_cost: z.coerce.number()
            .min(0.1, 'Min 1 cent of product cost'),
        price_sale: z.coerce.number()
    ...
    try {
        ...
        const { name, sku, quantity, category, min_quantity, max_quantity, price_cost, price_sale, supplier_id } = productSchema.parse(request.body)
        ...
        if(!createNewProduct){
            return reply.status(401).send('Error to create this product')
        }
        ...
        const errorMessage = e instanceof Error
            ? e.message : "Unknow errror to register this product"
